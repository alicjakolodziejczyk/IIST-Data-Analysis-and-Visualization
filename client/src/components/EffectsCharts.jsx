          ...
          {data.map((effect, index) => (
            <div key={index} style={{ display: 'grid', gridAutoFlow: 'column', gridTemplateColumns: '30% 70%' }}>
              ...
              <div style={{ padding: '10px', alignSelf: 'center' }}>
                <Progress past={(effect.przed/effect.wszystkie)*100+'%'} present={(effect.teraz/effect.wszystkie)*100+'%'} future={(effect.po/effect.wszystkie)*100+'%'} pastSubjects={effect.przedPrzedmioty} presentSubjects={effect.terazPrzedmioty} futureSubjects={effect.poPrzedmioty} />
              </div>
            </div>
          ))}
        </Accordion> 
